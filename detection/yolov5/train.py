...
from wrappedModels import SingleV5, V5Centernet, V5Dual
...
def train(hyp, opt, device, tb_writer=None):
    ...
    shutil.copyfile(os.path.basename(__file__), os.path.join(str(save_dir), os.path.basename(__file__)))
    # Configure
    ...
    # optimizer = MADGRAD(pg0,  lr=hyp['lr0'], momentum=hyp['momentum'])
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 11) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'total', 'labels', 'img_size', 'hm loss', 'lo_loss', 'loss'))
        print(('\n' + '%10s' * 11) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'total', 'labels', 'img_size', 'hm loss', 'lo_loss', 'loss'))
        if rank in [-1, 0]:
            pbar = tqdm(pbar, total=nb)  # progress bar
        optimizer.zero_grad()
        for i, (imgs, hms, targets, paths, _, logt) in pbar:  # batch -------------------------------------------------------------
            ni = i + nb * epoch  # number integrated batches (since train start)
            imgs = imgs.to(device, non_blocking=True).float() / 255.0  # uint8 to float32, 0-255 to 0.0-1.0

            hms = hms.to(device, non_blocking=True).float()

            # print(hms.shape, imgs.shape)
            # if i>10:
            #     break
            # print('output_layer====output_layer: ',torch.max(hms))
            ...
            with amp.autocast(enabled=cuda):
                pred, seg_out, logits = model(imgs)  # forward
                ...
                # print(hms.shape, seg_out.shape)

                # print(targets.shape, imgs.shape, logits.shape, paths.shape)
                # logits = torch.clip(logits, -11, 11)
                # print(logits)
                logit_loss = criterion(logits, logt.to(device))
            ...
            if rank in [-1, 0]:
                ...
                s = ('%10s' * 2 + '%10.4g' * 9) % (
                    '%g/%g' % (epoch, epochs - 1), mem, *mloss, targets.shape[0], imgs.shape[-1], hm_loss, logit_loss, loss)
                pbar.set_description(s)

                # Plot
                if plots and ni < 3:
                    f = save_dir / f'train_batch{ni}.jpg'  # filename
                    Thread(target=plot_images, args=(imgs, hms, targets, paths, f), daemon=True).start()
                    # if tb_writer:
...
if __name__ == '__main__':
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            ...
            print(f"{prefix}Start with 'tensorboard --logdir {opt.project}', view at http://localhost:6006/")
